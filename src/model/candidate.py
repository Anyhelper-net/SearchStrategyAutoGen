...
from src.io.anyhelper import (
    get_job_data,
    duplicate_screening,
    add_screening_resume,
    h5_post,
    get_user_role,
    get_ah_result_dict,
)
...
class Candidate:
    ...
    def set_simple_resume(self):
        self.simple_resume = bot_io.parse(
            bot_io.send(self.rawResume, ENUM_MODEL_ID.GET_SIMPLE_RESUME)
        )
    ...
    def set_first_check(self):
        ...
        self.firstCheckComment = bot_io.parse(
            bot_io.send(
                f"【个人信息：】{self.rawResume}【岗位要求：】{self.job_deep_requirement}",
    ...
    def set_fact_check(self):
        ...
        self.fact_check_comment = bot_io.parse(
            bot_io.send(
    ...
    def check_screening_is_duplicate_with_keywords(self):
        duplicate_response = get_ah_result_dict(
            duplicate_screening(self.liepin_keywords, self.incharge_id)
        )
        if "yes" == duplicate_response["duplicate"]:
            self.is_duplicate = True

            for duplicate_position in duplicate_response["records"]:
                self.duplicate_position_id_list.append(
                    duplicate_position["position_id"]
                )
    ...
    def upload_screening_resume_to_anyJob(self):
        ...
        clean_resume = bot_io.parse(
            bot_io.send(self.rawResume, ENUM_MODEL_ID.GET_CLEAN_RESUME)
        )
    ...
    def from_mm_raw_resume(cls, raw_resume_dict: dict, position_id, incharge_id=None):
        ...
        now_region = raw_resume_dict.get("province", "") + raw_resume_dict.get(
            "city", ""
        )
